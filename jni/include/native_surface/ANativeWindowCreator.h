        ...
        struct Functionals
        {
            ...
            Functionals(const SymbolMethod &symbolMethod)
            {
                ...
                if (!systemVersionString.empty()) {
                    // 无异常解析主版本号（NDK 常 -fno-exceptions）
                    // 兼容 "15" / "16" / "16.0" 等
                    int major = 0;
                    for (char c : systemVersionString) {
                        if (c >= '0' && c <= '9') major = major * 10 + (c - '0');
                        else break;
                    }
                    if (major > 0) systemVersion = static_cast<size_t>(major);
                }
                // 额外用 SDK 纠正（部分 OEM release 字符串异常）
                {
                    char sdk[32] = {};
                    if (__system_property_get("ro.build.version.sdk", sdk) > 0) {
                        int sdk_i = std::atoi(sdk);
                        if (sdk_i >= 36) systemVersion = 16;
                        else if (sdk_i >= 35) systemVersion = 15;
                        else if (sdk_i >= 34) systemVersion = 14;
                    }
                }
                ...
                static std::unordered_map<size_t, std::unordered_map<void **, const char *>> patchesTable = {
                    {
                        16,
                        {
                            // Android 16: LayerMetadata 在 android::gui，createSurface flags 为 int
                ...
                // 精确版本补丁；若无表（未来 17+）回退 16→15→14
                auto applyPatches = [&](size_t ver) {
                    auto it = patchesTable.find(ver);
                    if (it == patchesTable.end()) return false;
                    for (const auto &[patchTo, signature] : it->second) {
                        void *fn = symbolMethod.Find(libgui, signature);
                        if (fn) {
                            *patchTo = fn;
                        } else {
                ...
                if (!applyPatches(systemVersion)) {
                    if (systemVersion >= 16) {
                        if (!applyPatches(16)) applyPatches(15);
                    } else if (systemVersion == 15) {
                        applyPatches(14);
                    }
                }
                // 若 LayerMetadata ctor 仍空，再尝试 gui 命名空间
                if (systemVersion >= 14 && LayerMetadata__Constructor == nullptr) {
                    LayerMetadata__Constructor = reinterpret_cast<decltype(LayerMetadata__Constructor)>(
                        symbolMethod.Find(libgui, "_ZN7android3gui13LayerMetadataC2Ev"));
                }
                if (systemVersion >= 14 && LayerMetadata__setInt32 == nullptr) {
                    LayerMetadata__setInt32 = reinterpret_cast<decltype(LayerMetadata__setInt32)>(
                        symbolMethod.Find(libgui, "_ZN7android3gui13LayerMetadata8setInt32Eji"));
                }
        ...
        struct LayerMetadata {
            ...
            LayerMetadata() {
                if (9 < Functionals::GetInstance().systemVersion) {
                    auto fn = Functionals::GetInstance().LayerMetadata__Constructor;
                    if (fn) fn(data);
                }
            }
        ...
        struct SurfaceComposerClient {
            ...
            SurfaceControl CreateSurface(const char *name, int32_t width, int32_t height, bool skipScrenshot) {
                ...
                if (12 <= Functionals::GetInstance().systemVersion) {
                    ...
                    // TrustedOverlay 在 12+；符号缺失时跳过避免崩
